...
namespace Tiled
{
	...
	Tileset *getTilesetFromGid(Map *map, unsigned int gid)
	{
		std::vector<Tileset *> tilesets = map->getTilesets();

		for (std::vector<Tileset *>::const_reverse_iterator it = tilesets.rbegin(); it != tilesets.rend(); ++it)
		{
			Tileset *tileset = *it;
	...
	inline void createTileFromGid(Map *map, Layer *layer, unsigned gid, unsigned iterator)
	{
		Tileset *tileset = getTilesetFromGid(map, gid);

		if (!tileset)
			throw std::logic_error("Invalid tile gid : no tileset associated");
		...
		tile->setX((iterator % layer->getWidth()) * tileset->getTileWidth() + tileset->getOffsetX());
		tile->setY((iterator / layer->getWidth()) * tileset->getTileHeight() + tileset->getOffsetY());
